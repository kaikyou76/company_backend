...
public class XssProtectionTest extends SecurityTestBase {
        ...
        void testScriptTagInjectionPrevention() throws Exception {
                ...
                MvcResult result = mockMvc.perform(
                                put("/api/users/profile")
                                                ...
                                                .content(requestBody))
                                .andExpect(status().isOk()) // XSS content should be processed but escaped
                                .andReturn();
                ...
                for (String maliciousHandler : maliciousEventHandlers) {
                        ...
                        MvcResult result = mockMvc.perform(
                                        put("/api/users/profile")
                                                        ...
                                                        .contentType(MediaType.APPLICATION_JSON)
                                                        .content(requestBody))
                                        .andExpect(status().isOk()) // リクエストが正常に処理され、内容がエスケープされることを確認
                                        .andReturn();
                ...
                for (String maliciousUrl : maliciousUrls) {
                        ...
                        MvcResult result = mockMvc.perform(
                                        put("/api/users/profile")
                                                        ...
                                                        .contentType(MediaType.APPLICATION_JSON)
                                                        .content(requestBody))
                                        .andExpect(status().isOk()) // リクエストが正常に処理され、URLが無効化されることを確認
                                        .andReturn();
         ...
         * 
         * 前提条件:
         * - XSSプロテクションフィルターが有効である
         * - HTMLサニタイザーサービスが設定されている
         * 
         * 期待結果:
         * - HTML特殊文字が適切にサニタイズされる
         * - リクエストが正常に処理される（ブロックされない）
         * - <, >, &, ' が適切に処理される
         * - HTMLインジェクションが防がれる
         * - レスポンスが適切な形式で返される
         */
        ...
        void testHtmlResponseEscaping() throws Exception {
                ...
                MvcResult result = mockMvc.perform(
                                put("/api/users/profile")
                                                ...
                                                .content(requestBody)
                                                .with(csrf()))
                                .andExpect(status().isOk()) // リクエストが正常に処理され、HTML文字がエスケープされることを確認
                                .andReturn();
        ...
        void testJsonResponseSafety() throws Exception {
                ...
                MvcResult result = mockMvc.perform(
                                put("/api/users/profile")
                                                ...
                                                .header("Origin", "http://localhost:3000")
                                                .header("X-CSRF-TOKEN",
                                                                extractCsrfToken(mockMvc.perform(get("/api/csrf/token")
                                                                                .header("Authorization",
                                                                                                "Bearer " + validToken))
                                                                                .andReturn()))
                                                .contentType(MediaType.APPLICATION_JSON)
                                                .content(requestBody))
                                .andExpect(status().isOk()) // リクエストが正常に処理され、JSON内容がエスケープされることを確認
                                .andReturn();
        ...
        void testCombinedXssAttackPrevention() throws Exception {
                ...
                MvcResult result = mockMvc.perform(
                                put("/api/users/profile")
                                                ...
                                                .contentType(MediaType.APPLICATION_JSON)
                                                .content(requestBody))
                                .andExpect(status().isOk()) // リクエストが正常に処理され、複合攻撃がエスケープされることを確認
                                .andReturn();
         ...
         * 
         * 要件2.10対応: XSS防御処理のパフォーマンスが適切であること
         * 
         * 前提条件:
         * - XSS防御フィルターが有効である
         * - 大量のXSSペイロードが処理される
         ...
         * 期待結果:
         * - XSS防御処理時間が許容範囲内である
         * - 大量のペイロードでもシステムが安定している
         * - パフォーマンスが200ms以内で完了する
                        ...
                        try {
                                mockMvc.perform(
                                                put("/api/users/profile")
                                                                .header("Authorization", "Bearer " + validToken)
                                                                .header("Origin", "http://localhost:3000")
                                                                .header("X-CSRF-TOKEN", extractCsrfToken(mockMvc
                                                                                .perform(get("/api/csrf/token")
                                                                                                .header("Authorization",
                                                                                                                "Bearer " + validToken))
                                                                                .andReturn()))
                                                                .contentType(MediaType.APPLICATION_JSON)
                                                                .content(requestBody))
                                                .andExpect(status().isOk()); // リクエストが正常に処理され、XSS内容がエスケープされることを確認
                        } catch (Exception e) {
